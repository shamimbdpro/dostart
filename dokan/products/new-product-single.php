...
if ( ! isset( $post->ID ) && ! isset( $_GET['product_id'] ) ) {
	wp_die( esc_html__( 'Access Denied, No product found', 'digicart' ) );
}
...
if ( ! dokan_is_product_author( $post_id ) ) {
	wp_die( esc_html__( 'Access Denied', 'digicart' ) );
	exit();
}
			...
			<div class="dokan-dashboard-content dokan-product-edit">
				...
				<header class="dokan-dashboard-header dokan-clearfix">
					<h1 class="entry-title">
						<?php esc_html_e( 'Edit Product', 'digicart' ); ?>
						...
						<?php if ( $post->post_status == 'publish' ) { ?>
							<span class="dokan-right">
								<a class="dokan-btn dokan-btn-theme dokan-btn-sm" href="<?php echo esc_url( get_permalink( $post->ID ) ); ?>" ><?php esc_html_e( 'View Product', 'digicart' ); ?></a>
							</span>
						<?php } ?>

						<?php if ( $_visibility == 'hidden' ) { ?>
							<span class="dokan-right dokan-label dokan-label-default dokan-product-hidden-label"><i class="fa fa-eye-slash"></i> <?php esc_html_e( 'Hidden', 'digicart' ); ?></span>
						<?php } ?>
					</h1>
				</header><!-- .entry-header -->
				...
				<div class="product-edit-new-container product-edit-container">
					<?php if ( dokan()->dashboard->templates->products->has_errors() ) { ?>
						<div class="dokan-alert dokan-alert-danger">
							...
							<?php foreach ( dokan()->dashboard->templates->products->get_errors() as $error ) { ?>
								<strong><?php esc_html_e( 'Error!', 'digicart' ); ?></strong> <?php echo esc_html( $error ); ?>.<br>
							<?php } ?>
						</div>
					<?php } ?>
					...
					<?php if ( isset( $_GET['message'] ) && $_GET['message'] == 'success' ) { ?>
						<div class="dokan-message">
							...
							<strong><?php esc_html_e( 'Success!', 'digicart' ); ?></strong> <?php esc_html_e( 'The product has been saved successfully.', 'digicart' ); ?>

							<?php if ( $post->post_status == 'publish' ) { ?>
								<a href="<?php echo esc_url( get_permalink( $post_id ) ); ?>"><?php esc_html_e( 'View Product &rarr;', 'digicart' ); ?></a>
							<?php } ?>
						</div>
					<?php } ?>
						...
						if ( dokan_is_seller_enabled( get_current_user_id() ) ) {
							...
							<form class="dokan-product-edit-form" role="form" method="post">
								...
								<div class="dokan-form-top-area">
									...
									<div class="content-half-part dokan-product-meta">
										...
										<div class="dokan-form-group">
											...
											<label for="post_title" class="form-label"><?php esc_html_e( 'Title', 'digicart' ); ?></label>
											<?php
											dokan_post_input_box(
												$post_id,
												'post_title',
												array(
													'placeholder' => __( 'Product name..', 'digicart' ),
													'value' => $post_title,
												)
											);
											?>
											<div class="dokan-product-title-alert dokan-hide">
												<?php esc_html_e( 'Please enter product title!', 'digicart' ); ?>
											</div>
										</div>
										...
										<?php if ( is_array( $product_types ) ) : ?>
											<div class="dokan-form-group">
												<label for="product_type" class="form-label"><?php esc_attr_e( 'Product Type', 'digicart' ); ?> <i class="fa fa-question-circle tips" aria-hidden="true" data-title="<?php esc_html_e( 'Choose Variable if your product has multiple attributes - like sizes, colors, quality etc', 'digicart' ); ?>"></i></label>
										...
										<div class="show_if_simple dokan-clearfix">
											...
											<div class="dokan-form-group dokan-clearfix dokan-price-container">
												...
												<div class="content-half-part regular-price">
													<label for="_regular_price" class="form-label"><?php esc_html_e( 'Price', 'digicart' ); ?>
														<span
															...
															data-product-id="<?php echo esc_attr( $post_id ); ?>">
																( <?php esc_html_e( ' You Earn : ', 'digicart' ); ?><?php echo esc_html( get_woocommerce_currency_symbol() ); ?>
												...
												<div class="content-half-part sale-price">
													<label for="_sale_price" class="form-label">
														<?php esc_html_e( 'Discounted Price', 'digicart' ); ?>
														<a href="#" class="sale_schedule <?php echo esc_attr( $show_schedule ) ? 'dokan-hide' : ''; ?>"><?php esc_html_e( 'Schedule', 'digicart' ); ?></a>
														<a href="#" class="cancel_sale_schedule <?php echo ( ! $show_schedule ) ? 'dokan-hide' : ''; ?>"><?php esc_html_e( 'Cancel', 'digicart' ); ?></a>
													</label>
											...
											<div class="dokan-form-group dokan-clearfix dokan-price-container">
												<div class="dokan-product-less-price-alert dokan-hide">
													<?php esc_html_e( 'Product price can\'t be less than the vendor fee!', 'digicart' ); ?>
												</div>
											</div>
											...
											<div class="sale_price_dates_fields dokan-clearfix dokan-form-group <?php echo ( ! $show_schedule ) ? 'dokan-hide' : ''; ?>">
												<div class="content-half-part from">
													<div class="dokan-input-group">
														<span class="dokan-input-group-addon"><?php esc_html_e( 'From', 'digicart' ); ?></span>
														<input type="text" name="_sale_price_dates_from" class="dokan-form-control datepicker" value="<?php echo esc_attr( $_sale_price_dates_from ); ?>" maxlength="10" pattern="[0-9]{4}-(0[1-9]|1[012])-(0[1-9]|1[0-9]|2[0-9]|3[01])" placeholder="<?php esc_html_e( 'YYYY-MM-DD', 'digicart' ); ?>">
													</div>
												</div>

												<div class="content-half-part to">
													<div class="dokan-input-group">
														<span class="dokan-input-group-addon"><?php esc_html_e( 'To', 'digicart' ); ?></span>
														<input type="text" name="_sale_price_dates_to" class="dokan-form-control datepicker" value="<?php echo esc_attr( $_sale_price_dates_to ); ?>" maxlength="10" pattern="[0-9]{4}-(0[1-9]|1[012])-(0[1-9]|1[0-9]|2[0-9]|3[01])" placeholder="<?php esc_html_e( 'YYYY-MM-DD', 'digicart' ); ?>">
													</div>
												</div>
										...
										<?php if ( dokan_get_option( 'product_category_style', 'dokan_selling', 'single' ) == 'single' ) : ?>
											<div class="dokan-form-group">
												<label for="product_cat" class="form-label"><?php esc_html_e( 'Category', 'digicart' ); ?></label>
												<?php
												$product_cat = -1;
												$term = array();
												...
												$category_args = array(
													'show_option_none' => __( '- Select a category -', 'digicart' ),
													'hierarchical' => 1,
													'hide_empty' => 0,
													'name' => 'product_cat',
													'id'   => 'product_cat',
												...
												<div class="dokan-product-cat-alert dokan-hide">
													<?php esc_html_e( 'Please choose a category!', 'digicart' ); ?>
												</div>
											</div>
										<?php elseif ( dokan_get_option( 'product_category_style', 'dokan_selling', 'single' ) == 'multiple' ) : ?>
											<div class="dokan-form-group">
												...
												echo str_replace( '<select', '<select data-placeholder="' . esc_html__( 'Select product category', 'digicart' ) . '" multiple="multiple" ', $drop_down_category ); // phpcs:ignore WordPress.XSS.EscapeOutput.OutputNotEscaped
												?>
											</div>
										<?php endif; ?>

										<div class="dokan-form-group">
											<label for="product_tag" class="form-label"><?php esc_html_e( 'Tags', 'digicart' ); ?></label>
											<?php
											...
											$drop_down_tags = wp_dropdown_categories(
												array(
													'show_option_none' => '',
													'hierarchical' => 1,
													'hide_empty' => 0,
											...
											echo str_replace( '<select', '<select data-placeholder="' . esc_html__( 'Select product tags', 'digicart' ) . '" multiple="multiple" ', $drop_down_tags ); // phpcs:ignore WordPress.XSS.EscapeOutput.OutputNotEscaped
									...
									<div class="content-half-part featured-image">
										...
										<div class="dokan-feat-image-upload dokan-new-product-featured-img">
											...
											<div class="instruction-inside<?php echo esc_attr( $instruction_class ); ?>">
												...
												<a href="#" class="dokan-feat-image-btn btn btn-sm"><?php esc_html_e( 'Upload a product cover image', 'digicart' ); ?></a>
											</div>
															...
															if ( $gallery ) {
																foreach ( $gallery as $image_id ) {
																	...
																	<li class="image" data-attachment_id="<?php echo esc_attr( $image_id ); ?>">
																		<img src="<?php echo esc_url( $attachment_image[0] ); ?>" alt="Img">
																		<a href="#" class="action-delete" title="<?php esc_attr_e( 'Delete image', 'digicart' ); ?>">&times;</a>
																	</li>
																	<?php
																}
															}
															?>
															<li class="add-image add-product-images tips" data-title="<?php esc_html_e( 'Add gallery image', 'digicart' ); ?>">
								...
								<div class="dokan-product-short-description">
									<label for="post_excerpt" class="form-label"><?php esc_html_e( 'Short Description', 'digicart' ); ?></label>
									<?php
									wp_editor(
										$post_excerpt,
										'post_excerpt',
								...
								<div class="dokan-product-description">
									<label for="post_content" class="form-label"><?php esc_html_e( 'Description', 'digicart' ); ?></label>
									<?php
									wp_editor(
										$post_content,
										'post_content',
								...
								<input type="hidden" name="dokan_update_product" value="<?php esc_attr_e( 'Save Product', 'digicart' ); ?>"/>
								<input type="submit" name="dokan_update_product" class="dokan-btn dokan-btn-theme dokan-btn-lg dokan-right" value="<?php esc_attr_e( 'Save Product', 'digicart' ); ?>"/>
